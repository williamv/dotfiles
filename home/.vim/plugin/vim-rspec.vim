"
" Run spec in quickfix mode, like make
"
...
function! s:run_spec(path)
  let l:rubys = expand("~/").".vim/plugin/vim-rspec.rb"
  cexpr system("spec -r ".l:rubys." -f Spec::Runner::Formatter::VimFormatter " . path)
  cw
endfunction
...
function! Specs()
  ...
  call s:run_spec(l:dir)
endfunction

command! -nargs=? -complete=file Spec call Spec
command! -nargs=? -complete=file Specs call Specs
